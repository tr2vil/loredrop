...
def _call_tts(text, tts_settings):
    """Route TTS call to the configured provider. Returns audio bytes."""
    provider = tts_settings.get('provider', 'elevenlabs')

    if provider == 'edge_tts':
        from .edge_tts_client import text_to_speech
        return text_to_speech(
            text=text,
            voice=tts_settings.get('edge_voice', 'en-US-GuyNeural'),
            rate=tts_settings.get('edge_rate', '+0%'),
            pitch=tts_settings.get('edge_pitch', '+0Hz'),
        )
    else:
        ...
        if not voice_id:
            raise ValueError('No English voice ID configured. Set in Settings > TTS or .env.')
        return text_to_speech(
            text=text,
            voice_id=voice_id,
...
def generate_tts(selected_topic_id, run_id, log_fn=None):
    ...
    if log_fn:
        log_fn(f'TTS: Starting generation for {len(paragraphs)} paragraphs (provider: {provider})')
    ...
    for i, para in enumerate(paragraphs):
        ...
        # Call TTS provider
        audio_bytes = _call_tts(para.text, tts_settings)
...
def generate_single_tts(paragraph_id, run_id):
    """Generate TTS for a single paragraph. Returns dict with audio info."""
